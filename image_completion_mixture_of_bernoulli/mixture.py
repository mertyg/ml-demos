...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        N = np.sum(np.sum(R,axis = 0)+self.prior.a_mix-1)
        N_k = np.sum(R,axis=0)+self.prior.a_mix-1
        return N_k/N
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        N_k = (np.sum(R,axis=0)+self.prior.a_pixels+self.prior.b_pixels-2).reshape(-1,1)
        return (np.dot(R.T,X)+self.prior.a_pixels-1)/N_k
